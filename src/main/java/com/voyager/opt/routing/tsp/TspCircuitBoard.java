package com.voyager.opt.routing.tsp;
...
public class TspCircuitBoard {
    ...
    for (int fromNode = 0; fromNode < locations.length; fromNode++) {
      for (int toNode = 0; toNode < locations.length; toNode++) {
        if (fromNode == toNode) {
          ...
          distanceMatrix[fromNode][toNode] = (long) Math.hypot(
            locations[toNode][0] - locations[fromNode][0],
  ...
  static void printSolution(RoutingModel routing,
...
//      });
    ...
    final int transitCallbackIndex = routing.registerTransitMatrix(distanceMatrix);
